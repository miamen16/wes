...
function wes_assets() {
	...
	// Roboto (Latin) + Cairo (Arabic) from Google Fonts.
	wp_enqueue_style(
		'wes-fonts',
...
/**
 * Polylang serves the translated front page at the language home (e.g. /ar/) but
 * its permalink resolves to /ar/<slug>/, triggering a canonical redirect. Keep the
 * front page on the clean language-home URL.
 */
...
add_filter('acf/load_field/name=selected_tags_select', function($field) {
    // Get the currently selected taxonomy
...
// =============================================================
// AJAX Handler for Resource Cards Pagination
// =============================================================
...
function wes_rc_ajax_paginate() {
    // Public read-only endpoint: no nonce required (nonces break under page cache).

    // Page number
    $page = max( 1, intval( $_POST['page'] ?? 1 ) );

    // Set query variables so the template picks them up
    set_query_var( 'resource_page', $page );

    // Populate the global with all filter values, including CTA
    ...
    $wes_rc_ajax_filters = array(
        ...
        'action_types'   => array_filter( array_map( 'sanitize_text_field', explode( ',', $_POST['action_types'] ?? '' ) ) ),
        'audiences'      => array_filter( array_map( 'sanitize_text_field', explode( ',', $_POST['audiences'] ?? '' ) ) ),
    ...
    // Load the block template – ensure the path is correct
    get_template_part( 'template-parts/blocks/resource-cards-dynamic' );

    wp_die();
}
// =============================================================
// Enqueue AJAX script for Resource Cards
// =============================================================
add_action( 'wp_enqueue_scripts', 'wes_enqueue_resource_cards_ajax' );
function wes_enqueue_resource_cards_ajax() {
	if ( has_block( 'acf/wes-resource-cards-dynamic' ) ) {
		wp_enqueue_script(
			'wes-resource-cards-ajax',
			get_template_directory_uri() . '/assets/js/resource-cards-ajax.js',
			array( 'jquery' ),
			filemtime( get_template_directory() . '/assets/js/resource-cards-ajax.js' ),
			true
		);
		wp_localize_script( 'wes-resource-cards-ajax', 'wes_rc_ajax', array(
...
// =============================================================
// AJAX Handler for Resource Library Filtering
// =============================================================
...
function wes_ajax_drl_filter() {
    ...
    $show_flags   = isset( $_POST['show_flags'] ) ? filter_var( $_POST['show_flags'], FILTER_VALIDATE_BOOLEAN ) : true;
    ...
    // Set global settings for the template
...
// =============================================================
// Enqueue AJAX script for Resource Library
// =============================================================
add_action( 'wp_enqueue_scripts', 'wes_enqueue_resource_library_ajax' );
function wes_enqueue_resource_library_ajax() {
    if ( has_block( 'acf/resource-library-cpt' ) || has_block( 'acf/wes-resource-library-cpt' ) ) {
        wp_enqueue_script(
            'wes-resource-library-ajax',
            get_template_directory_uri() . '/assets/js/resource-library-ajax.js',
            array( 'jquery' ),
            filemtime( get_template_directory() . '/assets/js/resource-library-ajax.js' ),
            true
        );
        wp_localize_script( 'wes-resource-library-ajax', 'wes_drl_ajax', array(
            'ajaxurl' => admin_url( 'admin-ajax.php' ),
        ) );
    }
}
...
// =============================================================
// Multilingual Helpers for Get Involved
// =============================================================
...
// =============================================================
// AJAX Handler for Get Involved Filtering
// =============================================================
...
function wes_gi_ajax_filter() {
    ...
    if ( $query->post_count === 0 && $paged === 1 ) {
        $html = '<p class="gi__no-results">No resources found.</p>';
    }
...
function wes_gi_pagination( $query ) {
    ...
    <nav class="gi-pagination__nav">
        ...
        foreach ( $pages as $page ) :
            ...
            // Add dots when there is a gap.
            if ( $previous_page && $page > $previous_page + 1 ) :
                ?>
                <span class="gi-page-dots">...</span>
                <?php
            endif;
            ?>

            <button
                class="gi-page<?php echo $page === $current ? ' current' : ''; ?>"
...
// =============================================================
// Enqueue AJAX script for Get Involved
// =============================================================
add_action( 'wp_enqueue_scripts', function () {
    if ( is_front_page() ) {
        wp_enqueue_script(
            ...
            array( 'jquery' ),
            filemtime( get_template_directory() . '/assets/js/get-involved-ajax.js' ),
            true
        );
        wp_localize_script( 'wes-get-involved-ajax', 'wes_gi_ajax', array(
            'ajaxurl' => admin_url( 'admin-ajax.php' ),
        ) );
    }
} );
